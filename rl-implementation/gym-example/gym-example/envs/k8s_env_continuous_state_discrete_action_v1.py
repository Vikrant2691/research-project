...
class K8sEnvContinuousStateDiscreteActionV1(Env):
    metadata = {
        'render.modes': ['human']
    }

    def __init__(self, timestep_duration, app_name, sla_latency, prometheus_host,
            prometheus_latency_metric_name, cpu_thresh_init):
        ...
        # Observation:
        # [pod_cpu, cpu_hpa_threshold, pod_number, pods_backoff, pod_latency]
        self.observation_space = spaces.Box(
            np.array([0, 0, 1, 0, 0]),
            np.array([200, 100, self.MAX_PODS, self.MAX_PODS, 100 * self.sla_latency]),
            dtype=np.float64
        )
        # Possible actions for CPU thresholds:
    ...
    def reset(self):
        ...
        self._create_hpa(cpu_threshold)
    ...
    def _create_hpa(self, cpu_threshold):
        v2 = client.AutoscalingV2beta2Api()
